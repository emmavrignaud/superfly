...
# DEPRECATED module: post-hoc tracklet stitching is no longer in the active
# pipeline. Configuration now lives in legacy/stitching_config.yaml so it does
# not pollute the main config.yaml.
_CONFIG_PATH = Path(__file__).resolve().parent / "stitching_config.yaml"
if _CONFIG_PATH.exists():
    with open(_CONFIG_PATH) as f:
        cfg_stitching = yaml.safe_load(f) or {}
else:
    cfg_stitching = {}
