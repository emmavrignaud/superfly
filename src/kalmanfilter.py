...
--------------------
The filter maintains two distinct concepts:

  Observation — what the detector actually measures each frame: the bounding
  box [x1, y1, x2, y2], converted to [cx, cy, s, r] (centre, scale, aspect).
  Observations are direct measurements but noisy — the detector places the box
  slightly differently each frame even if the fly hasn't moved.

  State — the filter's internal best estimate of reality: [cx, cy, s, r, vx, vy, vs].
  It includes the four observable quantities PLUS their velocities, which are
  never directly measured — the filter infers them from how observations change
  over time.

  Analogy: tracking a car with a GPS that updates once per second.
    Observation = the GPS ping (position only, noisy).
    State = your best estimate of position AND speed. You infer speed from
    "it was here a second ago, now it's there."

The predict / update cycle
--------------------------
Every frame the filter alternates between two steps:

  predict() — before seeing the new frame, extrapolate forward using the
  current velocity: "the fly was moving right at 5px/frame, so it's probably
  5px further right now." This uses the state transition matrix F.

  update() — once the matched detection arrives, correct the prediction with
  the real measurement. The filter blends the two based on how much it trusts
  each source.

The key matrices
----------------
  F (state transition) — encodes the constant-velocity motion model.
  Multiplying F by the state gives the predicted next state:
    cx_new = cx + vx,  cy_new = cy + vy,  etc.

  H (observation matrix) — maps the 7D state down to the 4 things the
  detector sees. Velocities are hidden — H simply ignores them.

  Q (process noise) — how much do we expect the state to randomly deviate
  from the motion model? High Q = "the fly moves unpredictably, trust
  detections more than the prediction."
  *** This is a key tuning target for fly saccades — see ocsort.py. ***

  R (measurement noise) — how noisy are the detector's bounding boxes?
  High R = "the detector is imprecise, trust the prediction more."

Why this matters for broken tracks
-----------------------------------
If Q is too low, the filter over-commits to the linear prediction. A fly that
suddenly changes direction (saccade) will produce a detection far from the
prediction, IoU drops, the match fails, and a new ID is issued. Stitching then
has to repair this break. Tuning Q is the first upstream intervention.
"""
